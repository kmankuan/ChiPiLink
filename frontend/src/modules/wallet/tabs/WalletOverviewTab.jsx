/**
 * WalletOverviewTab — All users with wallet balances + stats + top-up/deduct
 */
...
export default function WalletOverviewTab({ token }) {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        totalBalance: list.reduce((s, u) => s + (u.wallet?.balance_usd || 0), 0),
        usersWithBalance: list.filter(u => (u.wallet?.balance_usd || 0) > 0).length
      });
    } catch {
  ...
  const handleAdjust = async () => {
    ...
    try {
      await axios.post(
        ...
        { amount: parseFloat(adjustAmount), action: adjustDialog.action, description: adjustDescription || undefined },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card><CardContent className="p-4 flex items-center gap-3">
          ...
          <p className="text-2xl font-bold" data-testid="total-users-stat">{stats.totalUsers}</p></div>
        </CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3">
          <DollarSign className="h-8 w-8 text-green-500" />
          <div><p className="text-sm text-muted-foreground">Total Balance</p>
          <p className="text-2xl font-bold" data-testid="total-balance-stat">${stats.totalBalance.toFixed(2)}</p></div>
        </CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3">
          <TrendingUp className="h-8 w-8 text-purple-500" />
          <div><p className="text-sm text-muted-foreground">With Balance</p>
          <p className="text-2xl font-bold" data-testid="users-with-balance-stat">{stats.usersWithBalance}</p></div>
        </CardContent></Card>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search users by name or email..." className="pl-10" data-testid="wallet-search-input" />
        </div>
        <Button variant="outline" onClick={fetchUsers} disabled={loading} data-testid="refresh-wallets-btn">
              ...
              {filtered.length === 0 ? (
                ...
                <TableRow key={user.user_id} data-testid={`wallet-row-${user.user_id}`}>
                  ...
                  <TableCell className="text-right">
                    <Badge variant={user.wallet?.balance_usd > 0 ? 'default' : 'secondary'}
                      className={user.wallet?.balance_usd > 0 ? 'bg-green-100 text-green-800' : ''}>
                      ${(user.wallet?.balance_usd || 0).toFixed(2)}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right text-sm">${(user.wallet?.total_deposited || 0).toFixed(2)}</TableCell>
                  <TableCell className="text-right text-sm">${(user.wallet?.total_spent || 0).toFixed(2)}</TableCell>
                  <TableCell>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="p-3 bg-muted rounded-lg">
              ...
              <p className="text-sm mt-1">Current balance: <span className="font-bold">${(adjustDialog?.user?.wallet?.balance_usd || 0).toFixed(2)}</span></p>
            </div>
            <div className="space-y-2">
              <Label>Amount (USD)</Label>
              <Input type="number" min="0.01" step="0.01" value={adjustAmount}
                onChange={(e) => setAdjustAmount(e.target.value)} placeholder="0.00" data-testid="adjust-amount-input" />
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdjust} disabled={adjusting || !adjustAmount || parseFloat(adjustAmount) <= 0}
