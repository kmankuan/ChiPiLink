 ...
 * Positioned on the scoring player's side (left or right)
 */
...
export default function EmotionOverlay({ emotion, settings, side = 'center' }) {
  ...
  useEffect(() => {
    ...
    setCurrent({ ...style, type: emotion.type, player: emotion.player });
    setVisible(true);
  ...
  // Position: left side, right side, or center (for deuce/match_point which affect both)
  ...
  return (
    <div className={`fixed inset-0 z-50 flex ${posClass} pointer-events-none`} style={{ background: current.bg }}>
      <div className={`text-center ${current.anim}`}>
        <div className="text-5xl mb-1">{current.label.split(' ')[0]}</div>
        <div className="text-lg font-black tracking-wide" style={{ color: current.color, textShadow: `0 0 20px ${current.color}40` }}>
          {current.label.slice(current.label.indexOf(' ') + 1)}
        </div>
      </div>
    </div>
  );
