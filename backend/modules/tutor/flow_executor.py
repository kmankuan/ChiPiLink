    ...
    try:
        ...
        while current_id:
            ...
            if node_type == "trigger":
                ...
                if not url:
                    return {"success": False, "error": "Scrape node missing URL."}
                
                execution_log.append(f"Scraping URL: {url} (Action: {action})")
                ...
                try:
                    async with async_playwright() as p:
                        ...
                        if action == "login":
                            ...
                            if user_sel:
                                await page.fill(user_sel, "test_student")
                            if pass_sel:
                                await page.fill(pass_sel, "test_password")
                            if submit_sel:
                        ...
                        context_data["scraped_text"] = content[:5000] # Limit to avoid token explosion
                        await browser.close()
                        execution_log.append(f"Successfully extracted {len(context_data['scraped_text'])} characters.")
