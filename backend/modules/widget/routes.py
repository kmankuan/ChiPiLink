...
# ── Widget Loader JS (served as application/javascript) ─────
...
async def widget_loader_js():
    ...
    api_url_env = __import__('os').environ.get('REACT_APP_BACKEND_URL', '')
...
(function() {{
  ...
  var API_URL = '{api_url_env}';
  var CONFIG = {__import__('json').dumps(config)};
  ...
  style.textContent = `
    ...
    #chipi-widget-btn {{
      ...
      border-radius: 999px;
      border: none;
      font-family: '{A.font_family || 'Inter'}', system-ui, sans-serif;
      font-size: 14px; font-weight: 600;
      color: #fff;
      background: ${{A.primary_color || '#16a34a'}};
  ...
  btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>' + (P.floating_label || 'ChiPi Link');
  ...
  frame.style.cssText = 'position:fixed; width:' + (P.sidebar_width || '380px') + '; height:100vh; top:0; right:-400px; box-shadow:-4px 0 30px rgba(0,0,0,0.15); border-radius:0;';
  ...
  function toggle() {{
    ...
    if (open) {{
      ...
      frame.style.right = '-' + (parseInt(P.sidebar_width) || 400) + 'px';
  ...
  // PostMessage listener for widget→parent communication
  window.addEventListener('message', function(e) {{
    if (e.data && e.data.type === 'chipi-widget-close') toggle();
    if (e.data && e.data.type === 'chipi-widget-resize') {{
      frame.style.width = e.data.width || P.sidebar_width || '380px';
    }}
  }});
