...
class CreateShipment(BaseModel):
    ...
    inventory_source: str = "public"  # "public" or "sysbook"
...
class CreateReturn(BaseModel):
    ...
    items: List[StockOrderItem]
    notes: Optional[str] = None
    inventory_source: str = "sysbook"  # Returns from linked orders default to sysbook
...
async def list_stock_orders(
    ...
    inventory_source: Optional[str] = Query(None, description="public|sysbook"),
    ...
    if status:
        query["status"] = status
    if inventory_source:
        query["inventory_source"] = inventory_source
    if search:
        query["$or"] = [
    ...
    if inventory_source:
        counts_pipeline.insert(0, {"$match": {"inventory_source": inventory_source}})
