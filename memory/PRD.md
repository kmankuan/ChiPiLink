...
- **Monday.com Order Status Sync (Mar 2, 2026)**: Added webhook handler for parent-item (order-level) status changes from Monday.com board. When the "Pedido Status" column is changed on the TB2026-Orders board (18397140868), it maps the Monday label to app status and updates the order. Default mapping: Recibido→submitted, Working on it/Procesando→processing, Listo/Ready→ready, Entregado/Delivered/Done→delivered, Cancelled/Cancelado→cancelled. Also triggers push notification to the user. Handler in `monday_textbook_adapter.py` method `handle_order_status_webhook`. Distinguishes parent vs subitem events by checking `parentItemId`. Integrated OneSignal push notifications for key user events. Frontend: `PushNotificationManager` component auto-links OneSignal identity to user on login (`setExternalUserId`), tags user role, and shows a subtle subscribe prompt 5s after login for unsubscribed users. Backend: `push_helpers.py` with fire-and-forget `notify_new_message()` and `notify_order_status()`. Push triggers added to: admin CRM reply, admin CRM topic create, and order status change endpoints. All push calls are non-blocking (`asyncio.create_task`).
