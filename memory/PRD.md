...
- 2026-04-19: **ChiPi Tutor module removed entirely from frontend** per user request (was causing recurring "X is not defined" crashes in production bundle — first ClubSchedule, then WorksheetGenerator). Deleted `/app/frontend/src/modules/tutor/` folder (10 components), removed all tutor imports/routes from `App.js`, removed `TutorDashboard` from `AdminDashboard.jsx`, removed tutor section from admin menu (`backend/modules/admin/menu_routes.py`). `/tutor/*` routes now redirect to `/`.
- 2026-04-19: **Tutor backend endpoints deprecated with `410 Gone`**. Removed all `modules.tutor.*` router includes from `main.py`. Added catch-all `api_router` (prefix `/tutor`) that responds `HTTP 410` with a JSON body `{error, code: "TUTOR_MODULE_DEPRECATED", message, deprecated_path}` to any method on any `/api/tutor/*` path. Removed tutor from the proxy middleware and stripped tutor-engine startup wiring. Stale clients (mobile caches, bookmarks, webhooks) now receive a clear deprecation signal.
